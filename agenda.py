...
#
# e-mails: gabriela.nori@aluno.faculdadeimpacta.com.br
# RA: 1901989
...
class Email:
    ...
    def email(self, email: str) -> None:
        ...
        if self.valida_email(email):
            self._email = email
            self._usuario = email[0:email.find('@')]
            self._dominio = email.replace(self._usuario+'@', '')
    ...
    def valida_email(email: str) -> bool:
        ...
        if not isinstance(email, str):
            raise TypeError('E-mail deverá ser string')
                        
        if (email.count('@') != 1):
            raise ValueError('E-mail deverá possuir somente uma @')

        email_validacao = email.replace('@', '')
        email_validacao = email_validacao.replace('.', '')

        if not email_validacao.isalnum():
            raise ValueError('E-mail deverá somente ter ponto, números, letras e @')
    ...
    def eh_impacta(self) -> bool:
        ...
        """
        return (self._dominio.find('faculdadeimpacta.com.br') >= 0)
...
class Contato:
    ...
    def adiciona_telefone(self, telefone: str, tipo='principal') -> None:
        ...
        self._telefones[tipo] = novoTelefone
    ...
    def lista_emails(self) -> List[Tuple[str, Email]]:
        ...
        para uma lista com list().
        """
        return list(self._emails.items())
    ...
    def buscar(self, valor_busca: str):
        ...
        telefones = str(self.lista_telefones())
        if (valor_busca in telefones):
            return True

        emails = str(self.lista_emails())
        if (valor_busca in emails):
            return True
...
class Agenda:
    ...
    def ligar(self, valor_busca, tipo='principal') -> None:
        ...
        """
        contatosEncontrados = self.busca_contatos(valor_busca)
    ...
    def apagar_contato(self, email_busca) -> str:
        ...
        for contato in self.contatos:
            emailsContato = contato.lista_emails()
            for email in emailsContato:
                if (email[1].email == email_busca):
                    self.contatos.remove(contato)
                    return str(contato) + ' excluído com sucesso!'
